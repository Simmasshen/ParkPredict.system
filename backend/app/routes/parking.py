...
def checkin():
    ...
    vehicle_plate = data.get("vehicle_plate")   # optional

    if not zone_id or not user_id:
        return jsonify({"success": False,
                        "error": "zone_id and user_id are required."}), 400

    result = check_in(zone_id=zone_id, user_id=user_id, vehicle_plate=vehicle_plate)
...
def user_history(user_id):
    ...
    logs = get_logs_by_user(user_id=user_id)
    return jsonify({"success": True, "data": logs}), 200
